    ...
    public class RemindMePickerCellViewModel : ObservableObject, IPickerCellViewModel
    {
        ...
        public RemindMePickerCellViewModel()
        {
            ...
            selectionChangedCommand = new Command((index) =>
            {
                ...
                if (selectedItem == OneWeekBefore)
                    App.SelectedModel.NextReminder = App.SelectedModel.NextDateToBeConsumed.AddDays(-7);
                if (selectedItem == OneMonthBefore)
                    App.SelectedModel.NextReminder = App.SelectedModel.NextDateToBeConsumed.AddMonths(-1);
            });
        }
        ...
        public int? GetSelectedIndex()
        {
            if (App.SelectedModel != null)
            {
                ...
                var date = App.SelectedModel.NextReminder.Subtract(App.SelectedModel.NextDateToBeConsumed);
                if (date.TotalDays == -1)
                    return 0;
                if (date.TotalDays == -7)
                    return 1;
